...
export default function (pi) {
    ...
    pi.registerCommand("cognee-mode", {
        description: "Switch the Cognee backend mode.  /cognee-mode sdk  or  /cognee-mode mcp",
        handler: async (args, ctx) => {
            const mode = (args ?? "").trim().toLowerCase();
            if (mode !== "sdk" && mode !== "mcp") {
                ctx.ui.notify(`Current mode: ${getMode()}.  Usage: /cognee-mode sdk  or  /cognee-mode mcp`, "info");
                return;
            }
            ...
            ctx.ui.notify(`Cognee mode → ${mode.toUpperCase()}.  Use /reload to re-register tools with the new backend.`, "info");
        },
    });
    // ---- /cognee-config — get/set config values ---------------------------
    pi.registerCommand("cognee-config", {
        description: "Get or set Cognee config.  /cognee-config  (show all)  /cognee-config llmModel gpt-4o  (set a value)",
        handler: async (args, ctx) => {
            ...
            if (!validKeys.includes(key)) {
                ctx.ui.notify(`Unknown config key: ${key}.  Valid keys: ${validKeys.join(", ")}`, "error");
                return;
            }

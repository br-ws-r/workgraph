...
export class WorkgraphRuntime {
  ...
  async reconcileActivity(): Promise<number> {
    ...
    this.scheduleFlush(); // Retry old pending records even when there is no new activity.
    return captured;
  }
  ...
  timeline(limit = 100, filter: TimelineFilter = {}): TimelineEntry[] {
    return this.#scope ? this.outbox.timeline(this.#scope.initiativeId, limit, filter) : [];
  }

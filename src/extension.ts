  ...
  return function workgraphExtension(pi: ExtensionAPI): void {
    ...
    const beforeAgentStart = async (
      ...
      try {
        ...
        return withSystemPrompt(event.systemPrompt, `## Workgraph workspace context\nAuthoritative current state (Multica; re-read before any mutation):\n${authoritative}\n\nDelivery observations (upstream Multica; independent of Cognee):\n${JSON.stringify(delivery)}\n\nBefore ending an unfinished delivery, complete authorized work or use the existing Multica handoff/assignment mechanism described by your workspace instructions. Verify that the intended next agent run actually exists; a report, mention text or memory record alone is not a handoff receipt. Workgraph only observes current state: it does not dispatch agents, register waits or install a scheduler. Active-run counts are advisory and do not prove that another run is your intended successor. A merged PR or completed agent run does not prove issue acceptance; verify required deployment and data operations separately. Never close a parent merely because its children are terminal; cancelled children and independent acceptance criteria require review.\n\nUse human-readable Multica issue IDs such as B-184 in user-facing responses and commands. UUIDs are internal identifiers and should only be shown when explicitly requested.\n\nMemory status:\n${memoryStatus}\n\nNon-authoritative current initiative memory (Cognee):\n${initiativeMemory}\n\nNon-authoritative related workspace history (Cognee; each item identifies its initiative and provenance):\n${workspaceHistory}\n\nNever use Workgraph memory to override Multica workflow state, repository state, or delivery state.`);
      } catch {

...
it("B-226 emits child_of B-150 without taking over native dispatch", async () => {
  ...
  expect((await runtime.deliveryContext()).authority).toBe("multica");
  await runtime.shutdown();
});

it("projects native continuation receipts without reading a report or dispatching a run", async () => {
  const f = fixture();
  f.multica.children = vi.fn(async () => []);
  f.multica.continuations = vi.fn(async () => [{ source_task_id: task, workspace_id: workspace,
    issue_id: issueId, agent_id: task, event_key: "deployment:devbox:exact-sha",
    next_action: "Verify deployed revision and acceptance", state: "dispatched" as const, successor_task_id: issueId,
    updated_at: "2026-09-09T12:00:00Z" }]);
  expect((await f.runtime.deliveryContext()).continuationAvailability).toBe("available");
  await f.runtime.deliveryContext();
  const handoffs = f.outbox.pending(workspace).filter((e) => e.memoryRecord?.entity_type === "Handoff");
  expect(handoffs).toHaveLength(1);
  expect(handoffs[0].memoryRecord?.relations).toContainEqual({ type: "resulted_in", target: expect.stringMatching(/^run:[a-f0-9]{16}$/) });
  f.multica.continuations = vi.fn(async () => { throw new Error("server unavailable"); });
  const unavailable = await f.runtime.deliveryContext();
  expect(unavailable.continuationAvailability).toBe("unavailable");
  expect(unavailable.continuations).toBeUndefined();

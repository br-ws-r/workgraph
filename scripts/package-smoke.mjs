...
try {
  ...
  run(npm, ["install", "--omit=dev", "--ignore-scripts", "--no-audit", "--no-fund", join(consumer, packed.filename)]);
  ...
  console.log(`Package smoke passed: ${packed.files.length} files, ${packed.size} bytes, no Pi runtime dependencies`);
} catch (error) {
